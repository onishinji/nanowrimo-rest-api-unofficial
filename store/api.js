    ...
    return new Promise(function(resolve, reject) {
        ...
        request.get(self.config.endpoint.replace(":username", id), function(error, response, body) {
            ...
            if (!error && response.statusCode == 200) {
                ...
                var nbDayRemaining = moment().daysInMonth() - moment().date() + 1
    ...
    return new Promise(function(resolve, reject) {
        ...
        request.get(self.config.endpoint.replace(":username", id), function(error, response, body) {
            ...
            if (!error && response.statusCode == 200) {
                ...
                var averageWordPerDay = parseInt($($("#cabin_stats dd")[0]).text().replace(",", ""));
                var wordcountToday = parseInt($($("#cabin_stats dd")[2]).text().replace(",", ""));
                var userWordToReach = parseInt($($("#cabin_stats dd")[3]).text().replace(",", ""));
                var userWordCount = parseInt($($("#cabin_stats dd")[4]).text().replace(",", ""));

                var nbDayRemaining = parseInt($($("#cabin_stats dd")[7]).text().replace(",", ""));
                var dailyTarget = parseInt($($("#cabin_stats dd")[9]).text().replace(",", ""));
                ...
                var history = arr[7].replace("[", "").replace("]", "").split(",");
